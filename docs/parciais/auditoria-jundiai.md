...
> **Nota de auditoria posterior (2026-08-01): duas lacunas abaixo foram resolvidas.** O WFS expõe atualmente **103 FeatureTypes** (contagem de snapshot; não 112) e o Observatório carrega dados por endpoints JSON públicos: 407 indicadores e 5.038 linhas de série, cobrindo 1991–2026. O parcial continua útil para os vazios por domínio, mas as afirmações de “painel sem API” e a contagem antiga não devem ser reutilizadas.
...
| Jundiaí/SP | multidomínio | Observatório Jundiaí — API usada pelo painel | `https://observatorio.jundiai.sp.gov.br/functions/api.php?sheet=indicadores` | JSON público | município e território conforme indicador | séries 1991–2026 | `[fetch-ok]` 408 linhas com cabeçalho, 407 indicadores e 41 colunas; endpoint de séries `Frontobservatório` com 5.039 linhas incluindo cabeçalho |
...
| Jundiaí/SP | urbanismo/mobilidade/ambiente | **GeoServer WFS público do GEOJundiaí** | `https://geo.jundiai.sp.gov.br/geoserver/ows?service=WFS&request=GetCapabilities` | WFS 2.0.0 (GetFeature, DescribeFeatureType) | lote, edificação, bairro, zona, equipamento | snapshot 2026-08-01 | `[fetch-ok]` **103 FeatureTypes** na contagem atual: zoneamento, lotes, edificações, endereçamento/CEP, bairros, ZEIS, bens tombados, bacias, parques, APA, ciclovias e terminais |
...
1. **A pendência "localizar endpoints do GEOJundiaí" está resolvida:** existe GeoServer WFS público com 103 FeatureTypes no snapshot atual. GetFeature devolve GeoJSON com esquema legível — recurso real aberto e lido.
2. **O Observatório também possui base pública.** O painel chama endpoints JSON sem autenticação: 407 indicadores, 41 colunas e 5.038 linhas de série, de 1991 a 2026. Isso eleva acesso e reprodutibilidade, mas não converte indicadores agregados em microdados operacionais.
...
Jundiaí sai desta auditoria **mais forte em acesso, território e governança, mas ainda desigual em dados operacionais**. O WFS atual tem 103 FeatureTypes e o Observatório fornece JSON público com indicadores e séries. Finanças mantém consultas declaradamente abertas. Saúde, educação, segurança e mobilidade ainda precisam de bases operacionais municipais além de indicadores e cadastros. Recomendação: manter Jundiaí como candidata principal, comparar formalmente com Niterói e não converter indicadores agregados em pontos de profundidade (`D`) sem dado subjacente.
