...
RISCO PARA O PROJETO 'Brasil iA': se o piloto for Belém, a maior parte do dado chega como .rar em Google Drive de terceiros e como painel — vocês vão precisar de camada de ingestão manual/semiautomática e de um plano B para o Anuário quebrado. Se for Porto Velho, a ingestão é trivial (API JSON documentada) mas o produto ao cidadão fica quase só fiscal, com apenas uma métrica de saúde real (fila de UPA — que, aliás, é ótima para 'informação simples e útil'). Considerem piloto duplo: Porto Velho para provar a esteira técnica, Belém para provar a amplitude temática.
